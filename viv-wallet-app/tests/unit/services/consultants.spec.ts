...
describe('ConsultantsService helper functions', () => {
    it('should normalize before saving', () => {
        ...
        } as Consultant).status).toBe("CONSULTANT_SENIOR_IN_ONBOARDING");
    });

    it('should read raw', () => {
        expect(normalizeRawConsultant({
            "startDate": "2020-09-12",
            ...
            "fullName": "Roger Flodin Java",
            "email": "roget.flondin.java@invivoo.com"
        }).status).toBe(ConsultantStatus.CONSULTANT_SENIOR_IN_ONBOARDING)
    });
})
